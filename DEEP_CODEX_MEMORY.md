...
- Added built-in read-only CLI smoke evals. `deepcodex evals list/show/run` exposes `repo-map`, `safety-smoke`, and `release-evidence` tasks; `evals run` forces `suggest` mode, uses the shared agent core, emits `eval_started`/agent events/`eval_result` in JSON mode, and keeps automatic scoring as future work. Verified `npm run build -w @deepcodex/cli`, `evals list --json`, `evals show safety-smoke --json`, and `evals run repo-map --json --max-steps 1` in local demo mode.
...
3. Add automatic eval scoring/history, richer DLP classification, kernel-level shell isolation, and policy-bundle distribution tooling beyond the current Web/Desktop status panel.
