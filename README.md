...
- `DEEPCODEX_PROVIDER_THINKING`: Defaults to `disabled`; when set to `enabled`, DeepCodex omits incompatible sampling/tool-choice controls and replays DeepSeek `reasoning_content` internally across tool-call turns.
...
- Provider diagnostics: `node apps/cli/dist/index.js providers ping --json`, open `http://127.0.0.1:17361/api/provider/ping`, or use the Web/Desktop right-rail Provider diagnostics panel.
...
- Web/Desktop provider diagnostics: use `Check provider` for configuration validation and `Live ping` only when intentionally testing DeepSeek connectivity.
