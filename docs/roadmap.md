...
| Phase 1: safer local agent | Make local write-mode use easier to trust. | Session replay UI, richer approval audit metadata, shell isolation, file-size limits. | User can approve each write or shell command with a visible diff or command body, and replay the decision later. |
...
| File-size limits | Prevents large files from overwhelming model context or memory. | Core, config. |
