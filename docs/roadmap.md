...
| Phase 1: safer local agent | Make local write-mode use easier to trust. | Session replay UI, richer approval audit metadata, shell isolation, binary-aware file handling. | User can approve each write or shell command with a visible diff or command body, and replay the decision later. |
