...
| --- | --- | --- | --- |
| Current interview slice | Demonstrate an end-to-end local coding agent product. | DeepSeek client, tool loop, token accounting, workspace guardrails, approval metadata, audit export, memory, Web session replay, Desktop, CLI, docs. | Checklist passes and limitations are documented. |
...
| Phase 2: product operations | Make demos repeatable and measurable. | Structured run history, audit retention controls, cost estimates, budget limits, benchmark tasks, regression evals. | A release can compare behavior across model/config changes. |
...
| Cost and budget controls | Makes provider spend predictable. | DeepSeek client, telemetry. |
