...
- `.deepcodex/policy-bundle.json` can bind the active config SHA-256 to an Ed25519 signature. CLI can generate a policy keypair, sign the active config with an external private key, and CLI/API verification checks bundles against one or more trusted public keys, optional issuer allowlists, and bundle/key revocation lists.
