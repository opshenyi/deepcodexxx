...
| `DEEPCODEX_PROVIDER_THINKING` | Optional. | `disabled` | DeepSeek V4 thinking toggle, `enabled` or `disabled`. DeepCodex defaults to `disabled` for predictable demos; enabled mode replays `reasoning_content` internally across tool-call turns. |
...
The current DeepSeek client sends non-streaming chat completion requests with tool definitions, `max_tokens: 4096`, and a 120 second timeout. In non-thinking mode it sends `temperature: 0.2` and explicit `tool_choice`; in thinking mode it omits incompatible sampling and `tool_choice` controls, can send `reasoning_effort`, and preserves `reasoning_content` inside the provider transcript for later tool-call turns. Product events are streamed by the local DeepCodex server even though the model request itself is not streamed. Provider calls retry 408, 429, 500, 502, 503, 504, and network failures with exponential backoff; when those retryable failures exhaust the retry budget for a model, the next approved fallback model is tried and a `provider_fallback` event is streamed and persisted. Provider failures are classified as authentication, authorization, request, not found, rate limit, timeout, server, network, invalid JSON, or unknown. 400-class request errors and invalid JSON are surfaced without retry or fallback.

DeepCodex sends `thinking: { "type": "disabled" }` by default for DeepSeek V4. Official DeepSeek docs say thinking mode defaults to enabled and that tool-call turns in thinking mode require `reasoning_content` to be passed back in later requests; otherwise the API can return 400. DeepCodex supports that internal replay when thinking is enabled, but keeps the default disabled for stable local demos, lower latency, and simpler audit review. Set `DEEPCODEX_PROVIDER_THINKING=enabled` or workspace `provider.thinking: "enabled"` for opt-in thinking runs; `DEEPCODEX_PROVIDER_REASONING_EFFORT` or `provider.reasoningEffort` can be `high` or `max`.
...
| Thinking-mode run fails after tool calls. | DeepSeek V4 thinking mode requires `reasoning_content` replay after tool-call turns, and rejects some non-thinking request controls. | Run `doctor` to confirm thinking settings, keep `DEEPCODEX_PROVIDER_THINKING=disabled` for stable demos, or capture the failing API payload shape before enabling thinking in release demos. |
| Provider error is classified as authentication or authorization. | The API key is missing, invalid, expired, or not permitted for the selected model/base URL. | Check `DEEPSEEK_API_KEY`, the base URL, workspace provider allowlists, and account access before changing agent logic. |
| Provider error is classified as rate limit, timeout, server, or network. | The provider or network path failed after bounded retries, and approved fallbacks may also have been exhausted. | Retry later, remove unhealthy fallbacks, or adjust retry settings only for trusted demos. |
