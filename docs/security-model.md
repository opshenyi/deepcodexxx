...
| Policy signing private key | External PEM file passed to CLI `config sign-bundle`. | Keep outside the workspace and do not store it in `.env`, `.deepcodex/config.json`, memory, session history, or repository files. |
