...
| DeepCodex to DeepSeek | API key is read from environment and sent as a bearer token to the configured base URL. Token usage is recorded when the provider returns usage metadata, optional token/cost budgets can stop further work after a limit is reached, pricing profiles are caller-managed configuration, the provider catalog exposes checked non-secret model metadata without prices, DeepSeek V4 thinking mode defaults to disabled for tool-loop compatibility, and `.deepcodex/config.json` can set workspace model, provider base URL, approved fallback models, provider/model allowlists, thinking mode, reasoning effort, and budget defaults. Signed policy bundles can verify the active workspace config before CLI/server runs when enforcement is enabled. | Add secrets management, key rotation, reasoning-content replay for thinking-mode tool calls, managed provider registry operations, and hosted provider operations. |
...
- DeepCodex sends DeepSeek V4 `thinking` as disabled by default. Enabling thinking mode is explicit because official DeepSeek docs require `reasoning_content` replay after tool-call turns, which is not yet persisted by this local agent loop.
...
| Provider catalog | Product source and local server response. | Non-secret model metadata only: model ids, status, fallback eligibility, migration target, retirement timestamp, and source checked date. It intentionally excludes keys, tenant data, and prices. |
| Policy signing private key | External PEM file generated by CLI `config generate-keypair` or passed to CLI `config sign-bundle`. | Keep outside the workspace and do not store it in `.env`, `.deepcodex/config.json`, memory, session history, or repository files. |
| Workspace configuration | `.deepcodex/config.json` in the selected workspace. | Non-secret team defaults only; explicit CLI/Web values and environment variables can override it. Config reads include a raw-file SHA-256 fingerprint. Web/Desktop display a non-secret policy summary from the local server, including provider allowlist and fallback counts, shell controls, DLP counts, artifact controls, retention, and config path. CLI can sign the config fingerprint into `.deepcodex/policy-bundle.json` with an external Ed25519 private key and export a trust package containing public-key metadata plus local/CI env values. Verification uses one or more trusted public keys, revocation lists, and issuer allowlists; `DEEPCODEX_REQUIRE_SIGNED_POLICY=true` can require verification before CLI/server agent runs. Web/Desktop can display the local server's verification result, but signing private keys stay out of browser and desktop renderer state. |
...
- Managed policy trust package distribution channels and audit reporting beyond the current CLI export and Web/Desktop status panel.
