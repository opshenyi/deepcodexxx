...
| CLI client | `doctor`, `ask`, `memory`, `evals`, and `sessions list/show/export` commands for terminal workflows. `ask --json` emits NDJSON event/result records for automation, `doctor --json` emits structured diagnostics with optional CI requirement gates for API key, workspace config, and trusted policy bundle, and built-in evals provide read-only smoke tasks with exact expected-signal scoring. | `apps/cli`, `docs/evals.md`. | Demo-ready. | Add shell completion, packaged binary polish, semantic eval scoring, and historical eval reports. |
...
- CLI built-in evals can run repeatable read-only smoke tasks, emit machine-readable eval records, and fail CI gates with `--min-score` or `--require-pass`.
