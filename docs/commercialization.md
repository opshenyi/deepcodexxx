...
| Web client | Browser console for workspace path, runtime server URL, execution mode, prompt, event stream, structured diff review, policy-bundle trust status, session replay, audit export, memory, and final output. | `apps/web`. | Demo-ready. | Add saved workspace profiles and richer side-by-side review affordances. |
