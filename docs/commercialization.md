...
| Web client | Browser console for workspace path, execution mode, prompt, event stream, structured diff review, session replay, audit export, memory, and final output. | `apps/web`. | Demo-ready. | Add saved workspace profiles, configurable server URL, and richer side-by-side review affordances. |
