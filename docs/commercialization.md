...
| CLI client | `doctor`, `ask`, `memory`, and `sessions list/show/export` commands for terminal workflows. `ask --json` emits NDJSON event/result records for automation, and `doctor --json` emits structured diagnostics with optional CI requirement gates for API key, workspace config, and trusted policy bundle. | `apps/cli`. | Demo-ready. | Add shell completion and broader packaging polish. |
