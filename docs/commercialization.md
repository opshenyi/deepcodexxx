...
| Web client | Browser console for workspace path, runtime server URL, saved workspace profiles, execution mode, prompt, event stream, unified/split structured diff review, policy-bundle trust status, session replay, audit export, memory, and final output. | `apps/web`. | Demo-ready. | Add richer review analytics and team-shared profile distribution. |
...
| CLI client | `doctor`, `ask`, `memory`, `evals`, and `sessions list/show/export` commands for terminal workflows. `ask --json` emits NDJSON event/result records for automation, `doctor --json` emits structured diagnostics with optional CI requirement gates for API key, workspace config, and trusted policy bundle, and CLI evals provide built-in plus workspace-defined read-only smoke tasks with exact expected-signal scoring, optional recorded history, and run-to-run comparison reports. | `apps/cli`, `docs/evals.md`. | Demo-ready. | Add shell completion, packaged binary polish, semantic eval scoring, and richer comparative reports. |
...
- CLI evals can run repeatable built-in and workspace-defined read-only smoke tasks, emit machine-readable eval records, optionally persist local eval history, compare recorded runs, and fail CI gates with `--min-score` or `--require-pass`.
