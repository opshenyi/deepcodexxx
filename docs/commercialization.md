...
| CLI client | `doctor`, `ask`, `memory`, `evals`, and `sessions list/show/export` commands for terminal workflows. `ask --json` emits NDJSON event/result records for automation, `doctor --json` emits structured diagnostics with optional CI requirement gates for API key, workspace config, and trusted policy bundle, and built-in evals provide read-only smoke tasks. | `apps/cli`, `docs/evals.md`. | Demo-ready. | Add shell completion, packaged binary polish, and automatic eval scoring. |
...
- CLI built-in evals can run repeatable read-only smoke tasks and emit machine-readable eval records.
