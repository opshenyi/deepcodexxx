...
| --- | --- | --- | --- | --- |
| Agent loop | DeepSeek-compatible chat completions loop with tool calls, bounded steps, provider retry/backoff for retryable failures, policy-controlled fallback models after retry exhaustion, classified provider failures, checked DeepSeek V4 model catalog metadata, explicit V4 thinking-mode control with internal `reasoning_content` replay for tool-call turns, token usage events, and run-level token/cost budgets when the provider returns usage metadata. Cost budgets can use direct prices or caller-managed pricing profiles. | `packages/core` agent, DeepSeek client, provider catalog, budget controls, pricing profiles, and tool registry. | Demo-ready. | Add managed provider registry operations and hosted provider operations. |
| Provider setup | Environment-driven DeepSeek API key and base URL, plus environment or workspace-config defaults for model, fallback models, thinking mode, reasoning effort, approved provider base URLs, approved models, team policy profiles, selected policy profile, pricing profile, max steps, custom redaction/DLP patterns, secret-write policy, shell network access, shell command allow/deny patterns, workspace config SHA-256 provenance, policy-bundle key generation, signing, verification, trust package export, CLI/API/Web model catalog views, provider ping diagnostics across CLI/API/Web/Desktop, Web/Desktop status display, key rotation/revocation controls, and env-controlled signed-only enforcement. Missing key falls back to local demo mode. | `.env.example`, `.deepcodex/config.json` loader, policy-bundle keypair/sign/verify/export CLI, server verification API, provider catalog API, provider ping API, provider policy enforcement, `DeepSeekClient`, CLI `doctor`, CLI `providers models/show/ping`, CLI `config show/init/generate-keypair/sign-bundle/verify-bundle/export-trust-package`, Web `Load config`, Web Provider catalog panel, Web Provider diagnostics panel, Web Policy bundle panel. | Demo-ready. | Add secrets management, managed trust package distribution channels, and hosted provider operations. |
...
| CLI client | `doctor`, `ask`, `memory`, `providers`, `evals`, `release evidence`, `release preflight`, `security scan`, `sessions list/show/export`, and `completion` commands for terminal workflows. `providers models/show` exposes checked DeepSeek V4 and legacy-alias metadata, `ask --json` emits NDJSON event/result records for automation, `doctor --json` emits structured diagnostics with optional CI requirement gates for API key, workspace config, and trusted policy bundle, CLI completion emits bash/zsh/PowerShell scripts or a JSON spec, and CLI evals provide built-in plus workspace-defined read-only smoke tasks with exact expected-signal scoring, optional recorded history, run-to-run comparison, and aggregated release reports. | `apps/cli`, `docs/evals.md`. | Demo-ready. | Add packaged binary polish, semantic eval scoring, and larger benchmark suites. |
...
7. Use Provider diagnostics to show configuration-only provider validation, then load release evidence, distribution preflight, eval evidence, and security scan panels to show release signals, delivery checks, and preflight safety checks; download the Markdown evidence reports for a reviewer handoff.
...
- Checked provider model metadata can be listed from CLI/API/Web without hard-coding provider prices.
...
- DeepSeek V4 thinking mode defaults to disabled for predictable demos, with env/workspace controls for opt-in thinking runs that preserve provider-required reasoning replay.
